...
export const mockShareholdersData: Record<string, Shareholder[]> = {
  GUJC000001: [
    {
      id: "sh-1",
...
export async function fetchShareholders(
  ...
  const shareholders = mockShareholdersData[applicationNo];
  if (!shareholders) return [];
  return shareholders;
}

export async function fetchShareholderById(
  applicationNo: string,
  ...
  const shareholders = mockShareholdersData[applicationNo];
  if (!shareholders) {
    throw new Error(`No shareholders found for application ${applicationNo}`);
  }
  const shareholder = shareholders.find((s) => s.id === shareholderId);
  if (!shareholder) {
    throw new Error(`Shareholder with ID ${shareholderId} not found`);
  }
  return shareholder;
}
